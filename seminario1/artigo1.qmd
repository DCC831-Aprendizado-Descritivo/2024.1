...
O artigo porém não se contém somente em discutir os algoritmos completos de mineração de padrões, mas também reconhece a importância de representações com um nível maior de significado. É nesse ponto em que são apresentados os algoritmos que mineram representações concisas dos subconjuntos de alta utilidade:
...
Com o fim de documetar o processo de instalação do Java, foi seguido o tutorial [deste link](https://www.java.com/pt-BR/download/help/download_options_pt-br.html). Entretanto, no momento da execução do SPMF o programa não funcionou e a solução foi reinstalar o JAVA de outra forma. Apenas a fim de registrar um possível erro que possa ser encontrado ao tentar executar o SPMF, fica aqui o vídeo do processo de instalação que **não funcionou**.
<figure align="center">
